...
#if DOSKIT
void (*GFX_EventsHandler)(void) = NULL;
Bitu (*GFX_GetBestModeHandler)(Bitu flags) = NULL;
Bitu (*GFX_GetRGBHandler)(Bit8u red, Bit8u green, Bit8u blue) = NULL;
Bitu (*GFX_SetSizeHandler)(Bitu width, Bitu height, Bitu flags, double scalex, double scaley, GFX_CallBack_t cb) = NULL;
bool (*GFX_StartUpdateHandler)(Bit8u * &pixels, Bitu &pitch) = NULL;
void (*GFX_EndUpdateHandler)(const Bit16u *changedLines) = NULL;
#endif
...
#if DOSKIT
    if (GFX_GetBestModeHandler) {
        return GFX_GetBestModeHandler(flags);
    }
#endif
	Bitu testbpp,gotbpp;
...
#if DOSKIT
    if (GFX_SetSizeHandler) {
        return GFX_SetSizeHandler(width, height, flags, scalex, scaley, callback);
    }
#endif
	if (sdl.updating)
		GFX_EndUpdate( 0 );
...
#if DOSKIT
    if (GFX_StartUpdateHandler) {
        return GFX_StartUpdateHandler(pixels, pitch);
    }
#endif
...
#if DOSKIT
    if (GFX_EndUpdateHandler) {
        GFX_EndUpdateHandler(changedLines);
    }
#endif
#if (HAVE_DDRAW_H) && defined(WIN32)
	int ret;
...
#if DOSKIT
    if (GFX_GetRGBHandler) {
        return GFX_GetRGBHandler(red, green, blue);
    }
#endif
	switch (sdl.desktop.type) {
	case SCREEN_SURFACE:
...
void GFX_Events() {
#if DOSKIT
    if (GFX_EventsHandler) {
        return GFX_EventsHandler();
    }
#endif
	SDL_Event event;
